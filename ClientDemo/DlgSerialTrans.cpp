...
void CTransparentTrans::OnCreatechannel() 
{
	...
	{
		g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_SerialStart serial[%d]", m_iSelSerialIndex);
		AfxMessageBox("Start Direct Transmission Failed");
		return;
	}
	g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_SerialStart serial[%d]", m_iSelSerialIndex);
	EnableControl(TRUE);
...
void CTransparentTrans::OnSenddata() 
{
	...
	m_lSerialChan = m_comboChan.GetCurSel()+1;	
