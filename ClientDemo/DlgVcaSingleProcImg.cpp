...
void CDlgVcaSingleProcImg::OnBtnSend() 
{
	...
	NET_VCA_FD_PROCIMG_RESULT struRecvBuf = {0};
	for (i=0; i<MAX_TARGET_NUM; i++)
	{
		struRecvBuf.struProcImg[i].pImage = new BYTE[10 * 1024];
		memset(struRecvBuf.struProcImg[i].pImage, 0, 10*1024);
	}
	if (NET_DVR_DetectImage(m_lServerID, &m_struFDProcImgCfg, &struRecvBuf))
	{
		g_pMainDlg->AddLog(m_iDevIndex, OPERATION_SUCC_T, "NET_DVR_DetectImage");
	}
	else
	{
		g_pMainDlg->AddLog(m_iDevIndex, OPERATION_FAIL_T, "NET_DVR_DetectImage");
		return;
	}
	
    if (struRecvBuf.dwSubImageNum > 0)
    {
		g_dwFaceNum = 0;
		for (i=0; i<struRecvBuf.dwSubImageNum; i++)
		{
			if (struRecvBuf.struProcImg[i].dwImageLen > 0 && struRecvBuf.struProcImg[i].pImage != NULL)
			{			
				...
				WriteFile(hFile, struRecvBuf.struProcImg[i].pImage, struRecvBuf.struProcImg[i].dwImageLen, &dwReturn, NULL);
				CloseHandle(hFile);
				hFile = NULL;
			}
			...
			{
				m_struFacePicInfo[g_dwFaceNum].dwImageID = struRecvBuf.dwImageId;
				m_struFacePicInfo[g_dwFaceNum].dwFaceScore = struRecvBuf.struProcImg[i].dwFaceScore;
				memcpy(&m_struFacePicInfo[g_dwFaceNum].struVcaRect, &struRecvBuf.struProcImg[i].struVcaRect, sizeof(NET_VCA_RECT));
				g_struFacePic[g_dwFaceNum].dwImageID = struRecvBuf.dwImageId;
				g_struFacePic[g_dwFaceNum].dwFaceScore = struRecvBuf.struProcImg[i].dwFaceScore;
				memcpy(&g_struFacePic[g_dwFaceNum].struVcaRect, &struRecvBuf.struProcImg[i].struVcaRect, sizeof(NET_VCA_RECT));
				g_dwFaceNum++;
			}
		}        
	}
	...
	for (i=0; i<MAX_TARGET_NUM; i++)
	{
		if (struRecvBuf.struProcImg[i].pImage != NULL)
		{
			delete []struRecvBuf.struProcImg[i].pImage;
		    struRecvBuf.struProcImg[i].pImage = NULL;
		}
	}
	if (sFileBuf != NULL)
	{
