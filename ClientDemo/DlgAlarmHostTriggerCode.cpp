...
BOOL CDlgAlarmHostTriggerCode::OnInitDialog() 
{
	...
	m_listTrigger.InsertColumn(0, szLan, LVCFMT_LEFT, m_rect.right/6);
	g_StringLanType(szLan, "状态", "Status");
	m_listTrigger.InsertColumn(1, szLan, LVCFMT_LEFT, m_rect.right/6);
	g_StringLanType(szLan, "触发器地址", "Trigger Address");
	m_listTrigger.InsertColumn(2, szLan, LVCFMT_LEFT, m_rect.right/6);
	g_StringLanType(szLan, "触发器通道", "Trigger Channel");
	m_listTrigger.InsertColumn(3, szLan, LVCFMT_LEFT, m_rect.right/6);
	g_StringLanType(szLan, "触发器名称", "Trigger Name");
	m_listTrigger.InsertColumn(4, szLan, LVCFMT_LEFT, m_rect.right/6);
	g_StringLanType(szLan, "触发器类型", "Trigger Type");
	m_listTrigger.InsertColumn(5, szLan, LVCFMT_LEFT, m_rect.right/6);
