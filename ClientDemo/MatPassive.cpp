    ...
    while (1)
    {
        ...
        else if (bNeedRead)
        {
            ...
            else if (dwRealSize > 0)
            {
				...
				if (!NET_DVR_MatrixSendData(g_struDeviceInfo[iDeviceIndex].struPassiveDecode[*lpSel].lPassiveHandle, tmpFile, tmpSize))
				{				
...
void CMatPassive::OnBtnAudiooff() 
{
    ...
    if (NET_DVR_MatrixPassiveDecodeControl(m_lServerID, m_DecChanCombo.GetCurSel() + m_lStartChan, &tmp))
    {
        MessageBox("Play AudioOff success!");
    }
    else
    {
        MessageBox("Play AudioOff failed!");
    }
}
...
void CMatPassive::OnBtnAudioon() 
{
    ...
    if (NET_DVR_MatrixPassiveDecodeControl(m_lServerID, m_DecChanCombo.GetCurSel() + m_lStartChan, &tmp))
    {
        MessageBox("Play Audioon success!");
    }
    else
    {
        MessageBox("Play Audiooff Success!");
