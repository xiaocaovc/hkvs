...
void CDlgHeatMapCfg::OnBtnPdcRulecfg() 
{
	...
	{        
		g_StringLanType(szLan,"设置热度图参数配置成功","Fail to Set Heat Map Param Info");
...
void CDlgHeatMapCfg::AddHeatMapDlg(char* lpInterBuff)
{
	...
	//空间热度图片长度
	memset(szStr, 0, sizeof(szStr));
	sprintf(szStr, "%d", lpInter->dwHeatMapPicLen);
	...
	{		
		...
		sprintf(cFilename, "%s\\LocalTime[%s].jpg",cFilename, chLocalTime);
...
void CDlgHeatMapCfg::OnBtnGetHeatmapInfo() 
{
	...
	m_struHeatMapQueryCond.dwChannel = m_lChannel;
