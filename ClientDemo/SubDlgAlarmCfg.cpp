...
BOOL CSubDlgAlarmCfg::CheckInitParam(BOOL bInit, int iInGroupNO, int iOutGroupNO)
{
 ...
 for (i=0; i<MAX_CHANNUM_V40; i++)
    {
        m_pbGetAlarmIn[i] = FALSE;
		m_pbSetAlarmIn[i] = FALSE;
    }
    
    for (i=0; i<MAX_ALARMIN_V40; i++)//m_dwAlarmInNum
	{	
		int iIPAlarmIndex = 0;
        
        if (i<(int)m_dwAlarmInNum || (i>= MAX_ANALOG_ALARMIN && m_pStruIPAlarmInCfgV40->struIPAlarmInInfo[i-MAX_ANALOG_ALARMIN].dwIPID > 0))
        {
...
void CSubDlgAlarmCfg::OnBnClickedBtnAlarmInHandle()
{
	...
	struLocalAlarmHandle.struAlarmHandleType.dwHandleType = m_pStruAlarmInCfgV40->dwHandleType;
	struLocalAlarmHandle.struAlarmHandleType.dwMaxRelAlarmOutChanNum = m_pStruAlarmInCfgV40->dwMaxRelAlarmOutChanNum;
	memcpy(struLocalAlarmHandle.struAlarmHandleType.dwRelAlarmOut, m_pStruAlarmInCfgV40->dwRelAlarmOut, sizeof(DWORD)*MAX_ALARMOUT_V40);
	...
	if (dlg.DoModal()==IDOK)
	{
		m_pStruAlarmInCfgV40->dwHandleType = struLocalAlarmHandle.struAlarmHandleType.dwHandleType;
		m_pStruAlarmInCfgV40->dwMaxRelAlarmOutChanNum = struLocalAlarmHandle.struAlarmHandleType.dwMaxRelAlarmOutChanNum;
		memcpy(m_pStruAlarmInCfgV40->dwRelAlarmOut, struLocalAlarmHandle.struAlarmHandleType.dwRelAlarmOut, sizeof(DWORD)*MAX_ALARMOUT_V40);
...
void CSubDlgAlarmCfg::OnBnClickedBtnAlarmInCopy()
{
	...
	if (m_iCopyAlarmIn == -1)
	{
		for (i=0; i<MAX_ALARMIN_V30; i++)
		{
			...
			memcpy(&(m_pStruAlarmInCfgV40[i]), &(m_pStruAlarmInCfgV40[m_iAlarmInIndex]), sizeof(NET_DVR_ALARMINCFG_V30));
			m_pbSetAlarmIn[i] = TRUE;
		}
	}
	else
	{	
		memcpy(&(m_pStruAlarmInCfgV40[m_iCopyAlarmIn]), &(m_pStruAlarmInCfgV40[m_iAlarmInIndex]), sizeof(NET_DVR_ALARMINCFG_V30));
