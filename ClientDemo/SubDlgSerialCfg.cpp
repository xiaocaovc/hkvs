	...
	, m_iSel232(0)
{
	...
	memset(&m_struRS232Cfg, 0, sizeof(m_struRS232Cfg));
    memset(&m_pStru485DecoderCfg, 0, sizeof(m_pStru485DecoderCfg));
    m_pStru485DecoderCfg = new NET_DVR_DECODERCFG_V30[MAX_CHANNUM_V40];
    if (m_pStru485DecoderCfg != NULL)
    {
		memset(m_pStru485DecoderCfg,0, sizeof(NET_DVR_DECODERCFG_V30) * MAX_CHANNUM_V40);
    }
...
// 	else
	...
	for (i=0; i <g_struDeviceInfo[m_iDeviceIndex].iDeviceChanNum ; i++)//PTZ parameter settings only valid for analog channels
	{
		...
		if (g_struDeviceInfo[m_iDeviceIndex].pStruChanInfo[i].bEnable)
		{
			...
			if (!NET_DVR_GetDVRConfig(m_lLoginID, NET_DVR_GET_DECODERCFG_V30, iChanShow, &m_pStru485DecoderCfg[i], sizeof(NET_DVR_DECODERCFG_V30), &dwReturned))
			{		
...
BOOL CSubDlgSerialCfg::CfgSetup()
{
	...
	for (i=0; i<g_struDeviceInfo[m_iDeviceIndex].iDeviceChanNum; i++)
	{
		if (m_pbSet485Serial[i] && g_struDeviceInfo[m_iDeviceIndex].pStruChanInfo[i].bEnable)
		{
			...
			if (!NET_DVR_SetDVRConfig(m_lLoginID, NET_DVR_SET_DECODERCFG_V30, iChanShow,&(m_pStru485DecoderCfg[i]), sizeof(NET_DVR_DECODERCFG_V30)))
			{		
				g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_SET_DECODERCFG_V30 %s", g_struDeviceInfo[m_iDeviceIndex].pStruChanInfo[i].chChanName); 
				g_StringLanType(szLan, "参数保存失败", "Save parameter failed");
				AfxMessageBox(szLan);	
				return FALSE;
			}
