...
void CDlgDeviceState::OnBnClickedBtnRefresh()
{
	...
	// 	if (!NET_DVR_GetDVRWorkState_V30(g_struDeviceInfo[m_iDeviceIndex].lLoginID, &struWorkState))
	// 	{
	// 		g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_GetDVRWorkState_V30");
	...
	case 2:
		...
		break;
	default:
		//ASSERT(pStruWorkStateV40->dwDeviceStatic < 3);
		break;
	}
...
// 		}
		...
		//所以需要转换一下，i按紧凑排列
		int iChannelIndex = g_struDeviceInfo[m_iDeviceIndex].pStruChanInfo[i].iChannelNO-1;
		...
		if (pStruWorkStateV40->struHardDiskStatic[i].dwVolume != 0)
		{
			...
			case 6:
				...
				m_csDiskState.Format(szLan);
 				break;
			default:
// 				ASSERT(pStruWorkStateV40->struHardDiskStatic[i].dwHardDiskStatic >= 0 &&\
// 					pStruWorkStateV40->struHardDiskStatic[i].dwHardDiskStatic < 4);
				break;
			}
		}else
		{
