...
void CDlgRemoteIPAccess::OnBtnAddIPChan() 
{
	...
	if (m_comboStreamType.GetCurSel() == 0)
	{
		...
		DWORD iDevID = 0;
		BOOL bFind = FindDataByIp(m_csDevIP.GetBuffer(0),&iDevID);
		if (!bFind)   //no find
		{
             if (iDevID == 0)
             {
		...
		{
			m_struIPParaCfgV40.struStreamMode[iIPChanIndex].uGetStream.struChanInfo.byIPID = (iDevID + iGroupNO*MAX_IP_DEVICE_V40) % 256;
			m_struIPParaCfgV40.struStreamMode[iIPChanIndex].uGetStream.struChanInfo.byIPIDHigh = (iDevID + iGroupNO*MAX_IP_DEVICE_V40) /256;
		}
...
/*********************************************************
  ...
  Return:	BOOL
**********************************************************/
BOOL CDlgRemoteIPAccess::FindDataByIp(char *lpDevIpAddress,  LPDWORD pdwIPDevIndex)
{
    UpdateData(TRUE);
