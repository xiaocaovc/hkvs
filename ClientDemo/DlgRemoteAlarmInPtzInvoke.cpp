...
BOOL CDlgRemoteAlarmInPtzInvoke::OnInitDialog() 
{
 ...
 m_bPreset = FALSE;
    ...
    {
        if ((m_nChannel - iStartNum) == m_lpAlarmInV40->struPresetChanInfo[i].dwEnablePresetChan)
        {
            m_bPreset = TRUE;
    ...
    {
        if ((m_nChannel - iStartNum) == m_lpAlarmInV40->struCruiseChanInfo[i].dwEnableCruiseChan)
        {
            m_bCruise = TRUE;
    ...
    {
        if ((m_nChannel - iStartNum) == m_lpAlarmInV40->struPtzTrackInfo[i].dwEnablePtzTrackChan)
        {
            m_bTrack = TRUE;
...
void CDlgRemoteAlarmInPtzInvoke::OnBnClickedBtnPtzOk()
{
    ...
    if (m_bPreset)
    {
		...
		for (i = 0; i < m_lpAlarmInV40->dwEnablePresetChanNum; i++)
		{
            if ( m_lpAlarmInV40->struPresetChanInfo[i].dwEnablePresetChan == m_nChannel - iStartNum)
            {
		...
		if (!bFindData)
		{
			m_lpAlarmInV40->struPresetChanInfo[m_lpAlarmInV40->dwEnablePresetChanNum].dwEnablePresetChan =m_nChannel - iStartNum;
		...
		FindDataInCruiseInfo(m_nChannel - iStartNum);  
		//find PTZ Info has this channel Info
		FindDataInPtzInfo(m_nChannel - iStartNum);
    }
    ...
    if (m_bCruise)
    {
        ...
        for ( i = 0; i < m_lpAlarmInV40->dwEnableCruiseChanNum; i++)
        {
            if (m_lpAlarmInV40->struCruiseChanInfo[i].dwEnableCruiseChan == m_nChannel - iStartNum)
            {
        ...
        if (!bFindData)
        {
            m_lpAlarmInV40->struCruiseChanInfo[m_lpAlarmInV40->dwEnableCruiseChanNum].dwEnableCruiseChan = m_nChannel - iStartNum;
        ...
        FindDataInPresetInfo(m_nChannel - iStartNum);
    ...
    if(m_bTrack)
    {
        ...
        for (i = 0; i < m_lpAlarmInV40->dwEnablePtzTrackChanNum; i++)
        {
            if ( m_lpAlarmInV40->struPtzTrackInfo[i].dwEnablePtzTrackChan == m_nChannel - iStartNum)
            {
                m_lpAlarmInV40->struPtzTrackInfo[i].dwPtzTrackNo = (DWORD)m_iPresetNo;
                bFindData = TRUE;
            }
        }
        ...
        if (!bFindData)
        {
            m_lpAlarmInV40->struPtzTrackInfo[m_lpAlarmInV40->dwEnablePtzTrackChanNum].dwEnablePtzTrackChan = m_nChannel - iStartNum;
