...
void CALLBACK g_GetGopInfoCallback(DWORD dwDataType, void* lpBuffer, DWORD dwBufLen, void* pUserData)
{
    ...
    else if (dwDataType == NET_SDK_CALLBACK_TYPE_DATA)
    {
        ...
        ::PostMessage(hWnd,WM_PROC_GOP_INFO_SHOW, (DWORD)(pStruRetInfo),dwBufLen);
...
BOOL CDlgGopInfo::OnInitDialog() 
{
    ...
    for (int i = 0; i< g_struDeviceInfo[m_iDevIndex].iDeviceChanNum; i++)
    {
        ...
        {
            m_cmbChan.AddString(g_struDeviceInfo[m_iDevIndex].pStruChanInfo[i].chChanName);
            m_cmbChan.SetItemData(dwIndex,g_struDeviceInfo[m_iDevIndex].pStruChanInfo[i].iChannelNO);
            dwIndex++;
        }
    }
