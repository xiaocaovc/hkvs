...
// 	}
	...
	if (m_pStruPicCfgV40[m_iCurChanIndex].byFontSize == 4)
	{
		g_StringLanType(szLan, "字体大小返回不支持的索引4,请更改字体大小配置!", \
			"The font size is not supported by the index returned 4, please change the font size configuration!");
		AfxMessageBox(szLan);
	}
...
void CSubDlgChanCfg::OnBnClickedBtnChanCfgOk()
{
	...
	case 1:
		...
		break;
	case 2:
		m_pStruRemoteRecCfg[m_iCurChanIndex].byStreamType = m_cmbStreamType.GetCurSel() + 1;
		break;
	default:
		break;
	}
...
// 	NET_DVR_SetDeviceConfig(m_lLoginID, NET_DVR_SET_MULTI_STREAM_COMPRESSIONCFG, 1, &struCond, sizeof(NET_DVR_MULTI_STREAM_COMPRESSIONCFG_COND), NULL, &struCompress, sizeof(NET_DVR_MULTI_STREAM_COMPRESSIONCFG));
	...
	for (i = 0; i < MAX_CHANNUM_V40; i++)
	{
		...
		if (m_pbSetChannelPara[i] && g_struDeviceInfo[m_iDeviceIndex].pStruChanInfo[i].bEnable)
		{
