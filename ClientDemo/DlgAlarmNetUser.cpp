...
BOOL CDlgAlarmNetUser::OnInitDialog() 
{
	...
	memset(&m_struDeviceUser, 0, sizeof(m_struDeviceUser));
	m_lChanNum = g_struDeviceInfo[m_iDevIndex].iDeviceChanNum;
...
//     (GetAlarmDeviceUser)(HPR_INT32 lUserID, LONG lUserIndex, NET_DVR_ALARM_DEVICE_USER* lpDeviceUser);
    if (NET_DVR_GetAlarmDeviceUser(m_lServerID, m_lUserIndex, &struDeviceUser))
    {
...
{
    if (NET_DVR_SetAlarmDeviceUser(m_lServerID, m_lUserIndex, &struDeviceUser))
    {
