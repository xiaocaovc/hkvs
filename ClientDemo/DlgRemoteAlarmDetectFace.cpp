	...
	, m_iMin82(0)
{
	...
	memset(&m_dwAlarmOut, 0xffffffff, sizeof(DWORD)*MAX_CHANNUM_V30);
	memset(&m_dwRecordChan, 0xffffffff, sizeof(DWORD)*MAX_CHANNUM_V30);
...
void CDlgRemoteAlarmDetectFace::OnClickTreeChan(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if (!bCheck)
	{
		m_dwRecordChan[dwIndex-1] = dwIndex;
	}
	else
	{
		m_dwRecordChan[dwIndex-1] = 0xffffffff;
	}
...
void CDlgRemoteAlarmDetectFace::OnClickTreeAlarmOut(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	BOOL bCheck = m_treeAlarmOut.GetCheck(hSelect);
 	if (!bCheck)
	{
		m_dwAlarmOut[dwIndex-1] = dwIndex;
	}
	else
	{
		m_dwAlarmOut[dwIndex-1] = 0xffffffff;
	}
