...
BEGIN_MESSAGE_MAP(CDlgCVR, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_ONE_KEY_CFG, OnButtonOneKeyCfg)
	ON_WM_TIMER()
...
void CDlgCVR::OnButtonOneKeyCfg() 
{
	// TODO: Add your control notification handler code here
	char szLan[128] = {0};

	m_lOneKeyCfgHandle = NET_DVR_StartRemoteConfig(m_lUserID, NET_DVR_ONE_KEY_CONFIG, NULL, 0, NULL, NULL);
	if (m_lOneKeyCfgHandle == -1)
	{
	...
	m_lOneKeyCfgTimer = SetTimer(ONE_KEY_CONFIG_TIMER, 500, NULL);
...
#endif
{
	...
	if (nIDEvent == ONE_KEY_CONFIG_TIMER)
    {
        DWORD dwState = 0;
        char szLan[128] = {0};
        NET_DVR_GetRemoteConfigState(m_lOneKeyCfgHandle, &dwState);
        if (dwState == NET_SDK_OKC_STATUS_SUCCESS)
        {
			...
			NET_DVR_StopRemoteConfig(m_lOneKeyCfgHandle);
			m_lOneKeyCfgHandle = -1;
			GetDlgItem(IDC_BUTTON_ONE_KEY_CFG)->EnableWindow(TRUE);
			KillTimer(ONE_KEY_CONFIG_TIMER);
        }
