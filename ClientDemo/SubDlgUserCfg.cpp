...
void CSubDlgUserCfg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDV_MaxChars(pDX, m_csUserName, 16);
...
//	else
	...
	if (!NET_DVR_GetDVRConfig(m_lLoginID, NET_DVR_GET_USERCFG_V40, 0, m_pStruUserCfg_V40, sizeof(NET_DVR_USER_V40), &dwReturned))
	{
...
BOOL CSubDlgUserCfg::CfgSetup()
{
	if (m_bSetUserParams)
	{
		if (!NET_DVR_SetDVRConfig(m_lLoginID, NET_DVR_SET_USERCFG_V40, 0,m_pStruUserCfg_V40, sizeof(NET_DVR_USER_V40)))
		{
			g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_SET_USERCFG_V40"); 
			char szLan[128] = {0};
...
// 				if (1 == m_pbLocalChan[i])
// 				{
// 					m_treeLocalChan.SetCheck(hChanItem, TRUE);
// 				}
// 			}
// 			else if (i >= dwStartDChanNo && m_iSelLocalRightType != 2)
// 			{
// 				strTemp.Format(DIGITAL_C_FORMAT, i+g_struDeviceInfo[m_iDeviceIndex].iStartChan-dwStartDChanNo);
// 				hChanItem =  m_treeLocalChan.InsertItem(strTemp, 0, 0, TVI_ROOT);
...
void CSubDlgUserCfg::CreateRemoteTree(void)
{
    ...
    DWORD dwStartDChanNo = g_struDeviceInfo[m_iDeviceIndex].pStruIPParaCfgV40[0].dwStartDChan;
	if (m_iChanCount <= 0)
	{
