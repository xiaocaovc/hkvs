		...
		if (0 == dwGroupNo)
		{
			...
			for (i = 0; i < (int)m_struHDCfg.dwHDCount; i++)
			{
				...
				case 3:
					...
					break;
				case 4:
					g_StringLanType(szLan, "RAID虚拟磁盘", "RAID虚拟磁盘");
					csTemp.Format(szLan);
					break;
				default:
					break;
...
void CDlgHardDiskCfg::UpdataChanStatus()
{
	...
	for (i=0; i<MAX_CHANNUM_V40; i++)
	{
		...
		{
			if (!m_struHDGroupCfg.struHDGroupAttr[m_iSelGroup].dwHDGroupNo)
			{
				m_bChkAllChan = FALSE;
				break;
			}
	...
	memset(m_pbHDGroupCfgV40,0, sizeof(BOOL)*MAX_CHANNUM_V40);
    for (i = 0; i < /*g_struDeviceInfo[m_dwDevIndex].iDeviceChanNum*/MAX_CHANNUM_V40; i++)
    {
        if (m_struHDGroupCfg.struHDGroupAttr[m_iSelGroup].dwRelRecordChan[i] != 0xffffffff && (m_struHDGroupCfg.struHDGroupAttr[m_iSelGroup].dwRelRecordChan[i] <= MAX_CHANNUM_V40))
        {
	...
	for (i=0; i<MAX_CHANNUM_V40; i++)
	{
		if (g_struDeviceInfo[m_dwDevIndex].pStruChanInfo[i].bEnable)
		{
			if (g_struDeviceInfo[m_dwDevIndex].pStruChanInfo[i].iChannelNO < g_struDeviceInfo[m_dwDevIndex].iDeviceChanNum + g_struDeviceInfo[m_dwDevIndex].iStartChan )
			{
				csTemp.Format(ANALOG_C_FORMAT, g_struDeviceInfo[m_dwDevIndex].pStruChanInfo[i].iChannelNO);
			}
			else
			{
				csTemp.Format(DIGITAL_C_FORMAT, g_struDeviceInfo[m_dwDevIndex].pStruChanInfo[i].iChannelNO - g_struDeviceInfo[m_dwDevIndex].pStruIPParaCfgV40->dwStartDChan + 1);
			}
...
void CDlgHardDiskCfg::SaveLastStatusToHDCfgV40(DWORD dwCurHDGroupNo)
{
    ...
    for (int i = 0; i < MAX_CHANNUM_V40; i++)
    {
        if (m_pbHDGroupCfgV40[i])
        {
            m_struHDGroupCfg.struHDGroupAttr[dwCurHDGroupNo - 1].dwRelRecordChan[dwCurRecordChanNum++] = i+1;
        }
    }
}
